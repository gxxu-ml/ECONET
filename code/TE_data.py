...
def create_data_instance(sent_id1, sent_id2, verb_id1, verb_id2, sentences, srl, event_coref):
    ...
    if row1.shape[0]:
        print('event_coref byte assertion success')
...
def get_core_data(dir='../may18_data/', story='cinderella-or-the-little-glass-slipper'):
    ...
    """
    srl_path = path.join(dir, story, story+'.srl.json')
    sentences_path = path.join(dir, story + '.sentences.csv')
    characters_path = path.join(dir, story + '.verb_subj_dobj.srl.csv')
...
def get_allen_srl(dir,story):
    ...
    for sent in sentences:
        ...
        srls.append(srl1)
    srl_dir = path.join(dir,story)
    if not path.exists(srl_dir):
        os.makedirs(srl_dir)
    with open(path.join(srl_dir,story+'.srl.json'), "w") as final:
        json.dump(srls, final)

def save_output_df(story_dir,srl,sentences,event_coref):
    """
...
def get_data(dir='../new_test/', story='cinderella-or-the-little-glass-slipper', get_srl=True):
    """
    :param dir: data dir
